...
impl RocksStorage {
    ...
    pub(crate) fn decode_data_row(bytes: &[u8], schema: &Schema) -> Vec<FieldValue> {
        ...
        for field in &schema.fields {
            let (v, consumed) =
                row::encoding::decode_value_with_len(&bytes[pos..], &field.data_type);
            result.push(v);
            pos += consumed;
        }
        result
...
mod tests {
    ...
    fn scan_with_range_bounds() {
        ...
        for i in 1..=5 {
            storage.insert_row(
                &t,
                vec![FieldValue::Int(i), FieldValue::Str(format!("r{}", i))],
            );
        }
